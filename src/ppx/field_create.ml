...
type r = [%t (Utils.get_type record_name.ptype_name.txt)]
]
...
let create_getter ~(record_name: type_declaration) label = 
  ...
  let arg_name = record_name.ptype_name.txt 
    |> Location.mknoloc
    |> Pat.var 
    |> Utils.fpat_constraint (Utils.get_type record_name.ptype_name.txt) in
  [%stri 
...
let create_setter ~(record_name: type_declaration) label =
  ...
  let get_record_arg_name = fun decls ->  
    let apply_type = Utils.fpat_constraint (Utils.get_type record_name.ptype_name.txt) in
    if(List.length decls > 1) then
      (
        record_name.ptype_name.txt 
          |> Location.mknoloc 
          |> Pat.var
          |> apply_type,
